...
- [x] R:R 风险收益比强制计算
- [x] SGE 时间错位警告
- [x] 三轴决策模型（AxisM/AxisL/AxisS）+ 30m/1H 执行门控解耦
- [x] 京东金融金友圈博主观点抓取（社区共识参考）
- [x] 自动生成 Markdown 报告文件并归档至 `assets/`
- [x] 多 Agent 工具适配（Cursor / Claude Code / OpenClaw / Gemini CLI）
...
1. **京东金融金友圈博主** — 黄金小小斌（信息观点每日聚合，省了好多搜索爬取的力气）、兰宫花匠（积存金点位方法论）

...
脚本策略：
- 优先用 Playwright 打开用户确认过的京东金页面，并按 XPath 抓取报价
- 若本机没有 Playwright 浏览器内核，或页面结构变动导致 XPath 失败，则自动回退到京东金 `latestPrice` 接口作为兜底参考
- 输出人类可读摘要 + 完整 JSON，方便后续接入主分析工作流

**没有 Playwright 又想读银行页面？** Cursor 内置浏览器走的是 **`cursor-ide-browser` MCP**，只能由 **当前 Chat 里的 Agent** 调用（`browser_navigate` → `browser_lock` → `browser_snapshot` → `browser_unlock`），终端里的 Python 脚本无法替你「自动打开」它。在对话里说明「用内置浏览器打开 SKILL 里的浙商/民生 URL 并读出价格」即可触发；详见 `SKILL.md` 阶段 2 中对应小节。
